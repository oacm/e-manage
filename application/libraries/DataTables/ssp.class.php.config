...
class SSP {
     ...
     */
    private $conf = "";
    private $configConnection = array(
        "default" => array(
            "user" => "root",
            "pass" => "@cc3ss/{}00Ftgyhummm",
            "db"   => "fenix_intranet_prd",
            "host" => "10.0.0.5"
        ),
        "dev"     => array(
            "user" => "root",
            "pass" => "F3N1X-w3bTI.prod#fnx",
            "db"   => "fenix_intranet_dvp",
            "host" => "localhost"
        ),
        "local"   => array(
            "user" => "root",
    ...
    public function getConnection() {
        return $this->configConnection[$this->conf];
    }
    ...
    static function data_output($columns, $data, $isJoin = false) {
        ...
        for ($i = 0, $ien = count($data); $i < $ien; $i++) {
            ...
            for ($j = 0, $jen = count($columns); $j < $jen; $j++) {
                $column       = $columns[$j];
                $column['as'] = str_replace("`", "", $column['as']);
                // Is there a formatter?
                if (isset($column['formatter'])) {
                    $row[isset($column['as']) ? $column['as'] : $column['dt']] = ($isJoin) ? $column['formatter']($data[$i][isset($column['as']) ? $column['as'] : $column['field']], $data[$i], isset($column['customData'])?$column['customData'] : NULL)  : $column['formatter']($data[$i][$column['db']], $data[$i], isset($column['customData'])?$column['customData'] : NULL);
                } else if (isset($column['as'])) {
                    $row[$column['as']] = ($isJoin) ? $data[$i][$column['as']] : $data[$i][$column['db']];
                } else {
                    $row[$column['dt']] = ($isJoin) ? $data[$i][$column['field']] : $data[$i][$column['db']];
                }
            }
            $out[] = $row;
        }
     ...
     *
     */
    static function simple($request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $specialCount = FALSE) {
        $bindings = array();
...
//        echo $query;
        ...
        $count_request = "SELECT COUNT(" . ($specialCount ? " DISTINCT " : "") . "{$primaryKey})";
        if ($joinQuery) {
